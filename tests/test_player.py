...
def test_match_history(mock, match_history_get_response):
    ...
    platform = ALPlatform.PC
    action = ALAction.GET
    history_url = base_url.format(version) + f"&platform={platform.value}&player={player_name}&history=1&action={action.value}"
...
def test_get_player(mock, basic_player_stats_response, match_history_get_response, match_history_info_response):
    player_name = "Player"
    platform = ALPlatform.PC
    player_url = base_url.format(version) + f"&platform={platform.value}&player={player_name}"
    history_get_url = base_url.format(version) + f"&platform={platform.value}&player={player_name}&history=1&action=GET"
    history_info_url = base_url.format(version) + f"&platform={platform.value}&player={player_name}&history=1&action=INFO"
    mock.register_uri('GET', player_url, json=basic_player_stats_response)
